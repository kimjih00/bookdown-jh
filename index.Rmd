...
# 주제선정이유
## 나이키와 아디다스가  대립 브랜드이고, 각 브랜드가 어떤 홍보활동이나 시장을 노리고 있는지 알아보고 싶어 그 차이를 분석해 보자 주제를 선정하게 되었습니다. 


# 데이터셋 설정 

## 빅카인즈에서 각 브랜드별 2021.10.01~현재까지의 기사들을 데이터 수집하였습니다. 
## 내용을 분석하는데 도움을 주는 패키지와 감성사전을 설치해 분석을 실시하려합니다. 
...
```
## 분석결과 
### 나이키에서는 메타버스 시장 관련 단어들이 많이 보이고, 
### 아디다스는 현재 진행되고 있는 카타르월드컵의 단어들이 많이 보입니다. 
### 아디다스 관련 기사 상위어에만 나이키가 있는 것을 보아 아디다스가 나이키와 비교되고 있는 것을 알 수 있습니다. 
...
#### 아디다스는 카타르 월드컵과, 아이돌과의 계약등이 눈에 띄는 토픽이었습니다.  아디다스는 구찌 같은 브랜드나, 아이돌과 협업하는 등의 노력을 볼 수 있었습니다. 
#### 아디다스도 나이키가 메타버스 시장에 관심을 가진 이후에 뛰어들게 되었으나, 주목도는 나이키에 비해 그리 크지 못합니다.  
...
#### 다양한 시장 환경에서 테니스가 나온 이유는 테니스메트로라는 테니스 샵에서 나이키의 품목을 할인해서 판매하기 때문입니다. 
...
### 코로나 이후 나이키는 메타버스 시장의 가능성을 보고 그 쪽 시장에 집중하는 경향을 보이고 있고, 이러한 메타버스 투자로 인해 주식관련 기사도 나오고, 미국증시가 여러 논란이 생기며 주요 톷픽으로 선정되었습니다. 
...
### 이를 통해 전체적인 브랜드 인지도나, 사람들의 선호도는 나이키가 더 높은 것으로 파악되었고, 이를 통해 아디다스의 홍보전략이 크게 효과적으로 나오지 않는 것 같습니다. 
